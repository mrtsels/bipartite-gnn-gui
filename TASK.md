...
**范围:** 按 2026-08-07 提交指引邮件逐项核对终期报告、海报、海报展示与布置。
**图例:** ✅ 已核对通过 / ⏳ 待用户执行（附截止）/ ⚠️ 有差异需确认 / ℹ️ 信息备忘
...
### A. 终期报告（截止 2026-08-12 Wed；VeriGuide 提前 ≥48h → 最晚 08-10 提交检查）

| # | 条目 | 状态 | 核对结果 |
|---|------|------|---------|
| A1 | 问题描述 + 为什么重要（Why Lightweight VLMs / Two Systematic Failure Modes） | ✅ | report.tex L146–277 |
| A2 | 现有方案总结（微调 / 检测器级联 / 生成模型 / GUI 模型） | ✅ | report.tex L308–362 |
| A3 | 新方案 + 结果（方法 4 头 + 5 组实验） | ✅ | report.tex L364–1031 |
| A4 | 非专家可读（单栏、背景自足） | ✅ | 17 页 letter，编译零 error |
| A5 | 与导师确认要求 | ✅ | Phase 12.4 已与 Prof Lau 沟通 |
| A6 | 参考文献 | ✅ | references.bib 14 条 |
| A7 | VeriGuide 检查（veriguide.org/login；Year 2025-2026 / Term 2 / ENGG-0001-A） | ⏳ | 用户执行，最晚 08-10 |
| A8 | VeriGuide 提交前删除参考文献页 | ⏳ | 用户执行 |
| A9 | 相似度 ≤25%（不含参考文献） | ⏳ | 待 VeriGuide 报告 |
| A10 | SharePoint 提交：完整报告（含参考文献）+ 合并 VeriGuide 报告 | ⏳ | 用户执行，最晚 08-12 |
| A11 | 文件名 "Alex Licheng Xie_Final Report" | ⏳ | 提交时命名 |
| A12 | 提交至个人文件夹 "Report and Poster" 栏 | ⏳ | 用户执行 |
| A13 | 立即检查 SharePoint 文件夹访问权限 | ⏳ | 用户执行 |
| A14 | 迟交罚款 HK$500/日历日 | ⚠️ | 勿逾期 |

### B. 海报（截止 2026-08-12 Wed）

| # | 条目 | 状态 | 核对结果 |
|---|------|------|---------|
| B1 | 尺寸 36"×48"（推荐） | ⚠️ | 当前 35.1"×46.8"（A0 高 × 3:4 宽比），在展板 37"×74.5" 内 ✓；如需严格 36×48 改 width=36,height=48 |
| B2 | 竖版 portrait | ✅ | 2527.8×3370.39 pt |
| B3 | CUHK 校徽 | ✅ | logos/CUHK-Logo.png |
| B4 | 项目标题 | ✅ | poster.tex L86 |
| B5 | 学生全名 Alex Licheng Xie | ✅ | L90 + PDF 元数据 |
| B6 | 导师姓名 Prof. Wing Cheong Lau | ✅ | L90 |
| B7 | 研究目标 | ✅ | Block 3 |
| B8 | 方法 | ✅ | Block 4 |
| B9 | 结果 | ✅ | Block 6 + 7 |
| B10 | 参考文献 | ✅ | poster.bib 14 条 |
| B11 | 文件名 "Alex Licheng Xie_Poster" | ⏳ | 提交时命名 |
| B12 | SharePoint 提交（个人文件夹 "Report and Poster" 栏） | ⏳ | 用户执行，最晚 08-12 |

### C. 海报展示（2026-08-14 Fri 9:30–12:00，ERBLT foyer 8/F ERB）

| # | 条目 | 状态 | 备注 |
|---|------|------|------|
| C1 | 强制出席，9:20 前到场签到 | ⏳ | 08-14 |
| C2 | 10–15 分钟展示 + Q&A（2 名 examiner） | ⏳ | 准备讲稿 |
| C3 | 仅 examiner / 参与者 / TA 可入内 | ℹ️ | — |

### D. 海报布置（2026-08-13 Thu 14:30–16:30）

| # | 条目 | 状态 | 备注 |
|---|------|------|------|
| D1 | 自行打印海报 | ⏳ | 08-13 前 |
| D2 | 13 日 14:30–16:30 到展板安装（现场提供别针） | ⏳ | 08-13 |
| D3 | 展板 74.5" 垂直 × 37" 水平，材料不得超出 | ✅ | 海报 46.8"×35.1" 在范围内 |
| D4 | 14 日 ~14:00 前拆除 | ⏳ | 遗留物将被移除 |
| D5 | 保留海报备后续使用 | ℹ️ | — |

### E. Best Project Award 评审标准（参考）
