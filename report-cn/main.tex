...
\hypersetup{colorlinks=true, linkcolor=black, urlcolor=blue, citecolor=blue,
    pageanchor=false}
% 中文排版不需要 emergencystretch（会放大标点后的胶水）
...
\begin{abstract}
能够“解读”手机屏幕的计算机程序，如视障用户辅助工具、自动化 UI 测试器和代替用户操作手机的软件代理，都需要知道屏幕上有什么、在什么位置。轻量级视觉语言模型（VLM）速度快、内存占用低，很适合在手机上直接运行这类任务，但它们会犯错：会漏掉 38\% 的可见界面元素，而且对检测到的元素所画的边界框往往与真实位置存在偏移。本报告提出一个后修正框架，在不重新训练 VLM、也不额外添加检测器的情况下修复这些错误。其基本假设是：设计良好的界面遵循空间规则——元素对齐、位于容器之内、间距一致。我们把规则编码为一张异构二分图：一侧是检测到的元素节点，另一侧是元素之间的空间约束节点，边只存在于两类节点之间。GraphSAGE 网络沿这张图传递消息，使每个元素的预测都能参考其空间邻居的信息，然后预测每个元素的坐标修正量、标记被违反的约束、判断每条检测是真实还是幻觉，并提出 VLM 完全漏掉的元素。在真实安卓截图数据集 RICO 上，模型检测违反约束的准确率达到 90--94\%；当大量元素缺失时，补全布局的 IoU 比最近邻基线最高提升 56\%。以 Qwen3-VL Flash 为前端在 200 张真实截图上端到端测试，修正后的流水线找回 106 个此前被漏掉的元素，F1 提升 2.0 个百分点、召回率提升 2.2 个百分点。修正模块运行在 VLM 之后，不修改底层模型，每张截图只增加不到一毫秒的推理时间。
\end{abstract}
...
\begin{itemize}
\item \textbf{元素遗漏。}38\% 的可见元素从未被报告：在 200 张真实 RICO 截图上，前端 VLM 只报告了 4789 个真实元素中的 2947 个（第~\ref{sec:endtoend}~节）。小图标、分隔线和嵌套容器最常被漏掉。
...
\begin{tikzpicture}[
  ...
  \node[font=\scriptsize, align=center, below=0.28cm of noisy] {38\% 遗漏，\\框偏移};
...
\subsection{真实 VLM 输出的端到端实验}\label{sec:endtoend}
...
\begin{tikzpicture}[scale=1.0, transform shape]
  ...
  \begin{scope}[shift={(0,0)}]
    ...
    % 对称误差条（std: 0.019, 0.018, 0.071）；标签在上帽上方
    ...
    \node[font=\scriptsize, rotate=90] at (-0.6,2.0) {违规准确率};
  ...
  \begin{scope}[shift={(6.6,0)}]
    ...
    % 对称误差条（std: 0.009, 0.009, 0.010）*30；标签在上帽上方
    ...
    \node[font=\scriptsize, rotate=90] at (-0.6,2.0) {提案 MSE};
    \node[font=\scriptsize, rotate=45, anchor=east] at (1.1,-0.45) {联合};
    \node[font=\scriptsize, rotate=45, anchor=east] at (2.5,-0.45) {仅违规};
    \node[font=\scriptsize, rotate=45, anchor=east] at (3.9,-0.45) {仅提案};
  \end{scope}
...
\section{讨论}
\subsection{结构推理何时有效}
纵观实验，结构推理在 VLM 输出稀疏（大量元素缺失）时帮助最大，在 VLM 已经准确时帮助较小。这呼应了最初促使图形式化的一连串负面结果。在模拟高斯噪声上，模型从未超过 no-op 基线；Qwen3-VL Plus 和 Flash 产生的位置误差在 0.01 左右或更小，没什么可修正；LLaVA-7B 每张截图只检测到几个元素，图承载不了多少结构；重加权的仅存在性目标也没有假阳性可过滤。只有当相当比例的元素缺失时，约束图才包含逐元素回归器无法触及的信息。对真实截图上的轻量级 VLM（漏掉 38\% 的元素）来说，这一情形正是实际应用相关的。
