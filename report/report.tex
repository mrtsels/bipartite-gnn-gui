...
\usepackage{setspace}
\onehalfspacing
% Superscript citations: [1], [2], ... raised above the text baseline
...
\hypersetup{colorlinks=true, linkcolor=black, urlcolor=blue, citecolor=blue,
    pageanchor=false,
    pdftitle={Heterogeneous Bipartite Graph Neural Networks for GUI Structure Error Correction},
    pdfauthor={Alex Licheng Xie},
    pdfcreator={Alex Licheng Xie},
    pdfproducer={Alex Licheng Xie}}
% Make the whole "[1]" (brackets included) a single hyperlink in
% superscript mode: suppress natbib's inner per-number link and wrap the
% full citation in one \hyper@natlinkstart/end pair. Must come after
% hyperref, which redefines \NAT@hyper@ on load.
\makeatletter
% In super mode, natbib's per-number link is suppressed here (the full
% citation is linked in \NAT@citesuper below). \NAT@cite@list holds the
% sorted citation keys at the time \NAT@citesuper runs.
\def\NAT@hyper@#1{%
  \ifNAT@super
    #1%
  \else
...
\centering
{\normalsize\textbf{Summer Research Internship Project}\par}
\vspace{0.9cm}
{\LARGE \textbf{Heterogeneous Bipartite Graph Neural Networks\\[0.15cm]for GUI Structure Error Correction}\par}
\vspace{0.6cm}
{\large \textit{A Post-Correction Framework for Lightweight Vision-Language Models}\par}
\end{minipage}
\vfill

% --- Author / Supervisor side by side ---
\begin{tabular}{@{}l@{\hspace{4.5cm}}l@{}}
\textbf{Author:} & \textbf{Supervisor:} \\
Alex Licheng Xie & Prof.\ Wing Cheong Lau \\
1155246441 [at] link.cuhk.edu.hk & wclau [at] ie.cuhk.edu.hk \\
\end{tabular}
\vspace{1.4cm}
...
\vfill
\end{titlepage}
% Cover page counts as page 1; contents page starts at page 2.
\setcounter{page}{2}
...
\begin{abstract}
Computer programs that interpret a phone screen, such as assistive tools for
...
are fast and memory-efficient, but they make mistakes: they miss 38\% of
...
VLM missed entirely. On the RICO dataset of real Android screenshots, the
...
\begin{itemize}
\item \textbf{Element omission.} 38\% of the visible elements are never
reported: on 200 real RICO screenshots, the front-end VLM reports only 2{,}947
of 4{,}789 ground-truth elements (Section~\ref{sec:endtoend}). Small icons,
dividers, and nested containers are missed most often.
...
\hyperref[fig:motivating]{Figure~\ref{fig:motivating}} shows the two failure modes on a real screenshot.
The left panel is the raw VLM output: the \texttt{OK} button's box is
misaligned with the button (red), and the \texttt{Share} button is missed
entirely (dashed circle). The right panel shows the corrected output after
the GNN: boxes are realigned and the missing element is proposed.
...
\draw[violet!80!black, fill=violet!12, line width=0.9pt] (12.15,8.11) rectangle (20.25,8.66);
\node[font=\scriptsize] at (16.2,8.38) {Recipe App};
\node[font=\scriptsize\bfseries, text=violet!70!black] at (20.00,8.16) {$\checkmark$};
\draw[violet!80!black, fill=violet!12, line width=0.9pt] (12.15,5.53) rectangle (20.25,7.72);
\node[font=\scriptsize] at (16.2,6.62) {img};
\node[font=\scriptsize\bfseries, text=violet!70!black] at (20.00,5.59) {$\checkmark$};
...
\end{tikzpicture}}
\caption{Motivating example. Left: raw VLM output with box misalignment
(red) and an omitted element (dashed circle). Right: the corrected output
after the GNN.}
...
the front-end model means retraining everything. Whatever errors survive
fine-tuning can only be fixed by training again.
...
new layouts from random noise or partial structure. Our problem is
different: we receive a noisy layout and must \emph{repair} it. The idea of
a learned layout prior carries over, but we make it explicit and
interpretable (ten spatial constraint types) rather than implicit in a
generative model.

\subsection{GUI-Specific Models}
A separate line of work builds models for GUI understanding directly.
SeeClick~\cite{cheng2024seclick} trains a VLM on instruction--coordinate
pairs: given a screen and a natural-language command, it predicts the target
position, and the ScreenSpot benchmark used in this report comes from the
same work. The front-end VLMs of this report, Qwen3-VL Flash and
MiniMax-VL-01, belong to the same family: they read a screenshot and output
elements and boxes.

What these models share is that accuracy is bought with size. Large ones are
accurate at grounding but run too slowly on a phone; small ones are fast and
produce the omission and misalignment errors described in
Section~I. A correction network does not change this trade-off. It runs
after the detector and repairs its output, so it can be combined with any of
these models instead of replacing one.
...
\subsection{Overview}
The pipeline is shown in \hyperref[fig:pipeline]{Figure~\ref{fig:pipeline}}. A screenshot is fed to a
...
\begin{tikzpicture}[
  ...
  \node[font=\scriptsize, align=center, below=0.28cm of noisy] {38\% omissions,\\box misalignment};
...
between elements. \hyperref[tab:constraints]{Table~\ref{tab:constraints}} lists the ten constraint types.
...
There are no element--element or constraint--constraint edges. \hyperref[fig:arch]{Figure~\ref{fig:arch}}
...
and ScreenSpot \cite{cheng2024seclick}. Implementation uses
...
\subsection{End-to-End on Real VLM Output}\label{sec:endtoend}
We deploy the trained model behind Qwen3-VL Flash on 200 real RICO
screenshots (4{,}789 ground-truth elements, 2{,}947 VLM predictions). The GNN
...
(\hyperref[tab:real]{Table~\ref{tab:real}}, \hyperref[fig:realvlm]{Figure~\ref{fig:realvlm}}).
...
Across the 32 screenshots with valid graphs, violated-constraint counts
track constraint counts almost linearly (\hyperref[fig:vcount]{Figure~\ref{fig:vcount}}, $r = 0.96$):
graphs with more extracted constraints also carry more violations to detect.
...
\begin{tikzpicture}[scale=1.0, transform shape]
  ...
  \draw[orange!70, line width=1.2pt] (0,{-1.4884*0.16}) -- ({35*0.14},{(0.8299*35-1.4884)*0.16});
  \node[font=\scriptsize, orange!80!black] at (4.0,4.15) {$r = 0.96$};
  \foreach \x/\y in {11/10,9/3,24/20,12/7,16/15,26/21,5/0,8/8,21/16,6/5,
                     21/16,34/29,23/15,9/7,32/20,14/9,9/3,4/3,28/23,15/7,
                     13/9,33/27,5/5,10/6,2/0,7/6,10/9,9/9,15/12,9/5,5/0,4/0} {
    \node[circle, fill=blue!60, inner sep=0pt, minimum size=0.18cm]
      at ({\x*0.14},{\y*0.16}) {};
  }
  % axes
  ...
  \node[font=\footnotesize, rotate=90] at (-1.05,2.4) {Violated constraints};
\end{tikzpicture}
\caption{Violated-constraint counts vs.\ extracted constraint counts across
32 RICO screenshots (OLS fit $y = 0.83x - 1.49$, $r^2 = 0.92$).}
\label{fig:vcount}
\end{figure}

\subsection{Constraint-Type Ablation}
Which spatial rules matter most? We ablate the ten constraint types on the
violation-detection task (\hyperref[tab:ablation]{Table~\ref{tab:ablation}}, \hyperref[fig:ablation]{Figure~\ref{fig:ablation}}).
...
\hyperref[fig:constraints]{Figure~\ref{fig:constraints}} quantifies the graph sizes behind
\hyperref[tab:ablation]{Table~\ref{tab:ablation}}: removing all alignment types leaves 15.1
constraints per graph versus 37.3 for the full set, which is why the
remaining violations are easier to classify.
...
\begin{tikzpicture}[scale=1.0, transform shape]
  ...
  \fill[blue!60] (\base,5.0) rectangle ({(\base+37.3*\sc)},5.35);
  ...
  \fill[gray!50] (\base,4.0) rectangle ({(\base+37.2*\sc)},4.35);
  \node[anchor=east, font=\footnotesize] at (\base-0.1,4.175) {Remove \texttt{GRID}};
  \node[font=\footnotesize, anchor=west] at ({(\base+37.2*\sc+0.4)},4.175) {37.2};
  % Remove SPACING 30.9 -> 3.71cm
  \fill[gray!50] (\base,3.0) rectangle ({(\base+30.9*\sc)},3.35);
  ...
  \fill[gray!50] (\base,2.0) rectangle ({(\base+28.7*\sc)},2.35);
  ...
  \fill[gray!50] (\base,1.0) rectangle ({(\base+22.4*\sc)},1.35);
  ...
  \fill[gray!50] (\base,0.0) rectangle ({(\base+15.1*\sc)},0.35);
...
\end{tikzpicture}
\caption{Average number of constraints extracted per graph under each
constraint set (n=500, drop=0.6).}
\label{fig:constraints}
\end{figure}

\subsection{Training-Objective Ablation}
We compare joint training against single-objective training (5 seeds,
\hyperref[fig:phase9]{Figure~\ref{fig:phase9}}). Joint training achieves the best balance: violation
...
\begin{tikzpicture}[scale=1.0, transform shape]
  ...
  \begin{scope}[shift={(0,0)}]
    ...
    % joint
    \fill[gray!50] (0.6,0) rectangle (1.6,{0.876*\sc});
    % violation-only
    \fill[gray!50] (2.0,0) rectangle (3.0,{0.898*\sc});
    % proposal-only
    ...
    \node[font=\scriptsize, rotate=90] at (-1.15,2.0) {Violation accuracy};
  ...
  \begin{scope}[shift={(6.6,0)}]
    ...
    \fill[gray!50] (0.6,0) rectangle (1.6,{0.051*\scm});
    \fill[blue!60] (2.0,0) rectangle (3.0,{0.116*\scm});
    \fill[gray!50] (3.4,0) rectangle (4.4,{0.051*\scm});
    ...
    \draw (1.02,{(0.051+0.009)*\scm}) -- (1.18,{(0.051+0.009)*\scm});
    \node[font=\scriptsize] at (1.1,{(0.051+0.009)*\scm+0.18}) {0.051};
    \draw (2.5,{(0.116-0.009)*\scm}) -- (2.5,{(0.116+0.009)*\scm});
    \draw (2.42,{(0.116-0.009)*\scm}) -- (2.58,{(0.116-0.009)*\scm});
    \draw (2.42,{(0.116+0.009)*\scm}) -- (2.58,{(0.116+0.009)*\scm});
    \node[font=\scriptsize] at (2.5,{(0.116+0.009)*\scm+0.18}) {0.116};
    \draw (3.9,{(0.051-0.010)*\scm}) -- (3.9,{(0.051+0.010)*\scm});
    \draw (3.82,{(0.051-0.010)*\scm}) -- (3.98,{(0.051-0.010)*\scm});
    \draw (3.82,{(0.051+0.010)*\scm}) -- (3.98,{(0.051+0.010)*\scm});
    \node[font=\scriptsize] at (3.9,{(0.051+0.010)*\scm+0.18}) {0.051};
    \draw[thick,-{Stealth}] (0,-0.15) -- (0,4.1);
    \draw[thick,-{Stealth}] (-0.15,0) -- (4.6,0);
    % y-axis ticks and label
    \foreach \v in {0.04,0.08,0.12} {
      \draw (-0.1,{\v*\scm}) -- (0.1,{\v*\scm});
      \node[anchor=east, font=\tiny] at (-0.15,{\v*\scm}) {\v};
    }
    \node[font=\scriptsize, rotate=90] at (-1.15,2.0) {Proposal MSE};
    \node[font=\scriptsize, rotate=45, anchor=east] at (1.1,-0.45) {joint};
    \node[font=\scriptsize, rotate=45, anchor=east] at (2.5,-0.45) {violation-only};
    \node[font=\scriptsize, rotate=45, anchor=east] at (3.9,-0.45) {proposal-only};
  \end{scope}
...
copies the box of the closest surviving element (\hyperref[fig:completion]{Figure~\ref{fig:completion}}).
...
MacBook Pro powered by an M4 Pro chip, negligible compared with VLM inference at roughly two seconds
...
lightweight VLMs on realistic screenshots (which omit 38\% of elements),
