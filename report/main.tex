...
% Based on the Phase 12 paper (paper/main.tex), expanded for a general
% technically-literate audience. Single column, 12pt.
...
\author{\IEEEauthorblockN{Xie Licheng (Alex)}
...
\and
\IEEEauthorblockN{Prof.\ Wing Cheong Lau}
\IEEEauthorblockA{\textit{Department of Information Engineering} \\
\textit{The Chinese University of Hong Kong}\\
Hong Kong SAR, China \\
...
\markboth{UG Summer Research Internship 2026 --- Final Report}{Xie \& Lau}
...
VLM or adding a second detector. The key insight is that well-designed
interfaces obey spatial rules --- elements align, sit inside containers, and
follow consistent spacing. We encode these rules as a heterogeneous bipartite
graph: one set of nodes for the detected elements, one set for the spatial
constraints among them, and edges only between the two sets. A GraphSAGE
neural network passes messages along this graph so that every element's
prediction is informed by its spatial neighbours, then predicts per-element
coordinate corrections, flags violated constraints, scores whether each
detection is real or hallucinated, and proposes elements the VLM missed
entirely. On the RICO dataset (real Android screenshots), the model detects
violated constraints at 90--94\% accuracy and, when a large fraction of
elements is missing, completes the layout with up to 56\% better IoU than a
nearest-neighbour baseline. End-to-end on 200 real screenshots with a Qwen3-VL
Flash front end, the corrected pipeline recovers 106 previously missed
elements, raising F1 by 2.0 points and recall by 2.2 points. The framework
is model-agnostic, requires no VLM fine-tuning, and adds less than one
millisecond of inference per screenshot.
\end{abstract}

\begin{IEEEkeywords}
GUI understanding, graph neural networks, vision-language models,
structure correction, bipartite graphs, accessibility
\end{IEEEkeywords}

\section{Introduction}
\IEEEPARstart{T}{o} understand a smartphone interface, a program must first
...
Careful evaluation on real screenshots shows that lightweight VLMs make two
kinds of mistakes:
...
compressing a large model into a small one. The natural response --- train a
bigger model --- is precisely what on-device constraints forbid.

\subsection{Our Approach in One Paragraph}
We observe that GUI screenshots are \emph{highly structured}: designers lay
out elements according to spatial rules such as alignment, containment,
spacing, and grid membership. These rules are independent evidence that a
VLM looking at pixels alone cannot exploit. If every element in a row is
left-aligned except one, the odd one out is probably misplaced --- regardless
of what the pixels look like. We formalize these rules as a graph: element
nodes on one side, constraint nodes on the other, and edges connecting an
element to every constraint it participates in. A graph neural network
...
runs after the VLM, it is \emph{model-agnostic}: the same correction network
works behind any VLM, and it never needs the VLM's weights.
...
This improves accuracy but has two costs: it requires labelled GUI data, and
...
they synthesize complete layouts. Our problem is different --- we receive a
...
The framework proposed here differs from all of the above in a fundamental
way: it never generates pixels or detections from scratch, and it never
touches the VLM. It \emph{post-processes} the VLM's noisy output using
structural reasoning. This keeps the on-device cost of the VLM unchanged,
adds only a small graph network (57K parameters), and works with any VLM that
outputs a list of elements with bounding boxes.
...
\end{tikzpicture}
\caption{End-to-end pipeline: screenshot $\rightarrow$ lightweight VLM
$\rightarrow$ noisy JSON $\rightarrow$ heterogeneous bipartite graph
$\rightarrow$ GraphSAGE $\rightarrow$ corrected JSON.}
\label{fig:pipeline}
\end{figure}
...
\texttt{CENTER\_X} & center-distance $<\varepsilon$ & Horizontally centred \\
\texttt{CENTER\_Y} & center-distance $<\varepsilon$ & Vertically centred \\
...
\begin{tikzpicture}[
  ...
  \node[cons] (c1) at (5.2,2.0) {$c_1$: ALIGN\_LEFT};
  \node[cons] (c2) at (5.2,0.9) {$c_2$: CONTAINMENT};
  \node[cons] (c3) at (5.2,-0.2) {$c_3$: SPACING};
  \node[cons] (c4) at (5.2,-1.3) {$c_4$: SAME\_SIZE};
...
aggregates neighbouring features with a mean operation.
...
with $\hat{e}_i=\sigma(\operatorname{MLP}_{\text{exist}}(\mathbf{h}_{e_i}^{(2)}))$:
\begin{equation}
...
\begin{tikzpicture}[scale=1.0, transform shape]
  ...
  \def\sc{60}
  \def\base{0.0}
  % y positions for 6 rows (top to bottom), bar height 0.35
  % All 10 types (control) 0.908 -> 4.08cm
  \fill[blue!60] (\base,5.0) rectangle ({(\base+ (0.908-0.84)*\sc)},5.35);
  \node[anchor=east, font=\footnotesize] at (\base-0.1,5.175) {All 10 types};
  \node[font=\footnotesize, anchor=west] at ({(\base+(0.908-0.84)*\sc+0.1)},5.175) {0.908};
  % Remove CONTAINMENT 0.889 -> 2.94cm
  \fill[gray!50] (\base,4.0) rectangle ({(\base+(0.889-0.84)*\sc)},4.35);
  \node[anchor=east, font=\footnotesize] at (\base-0.1,4.175) {Remove CONTAINMENT};
  \node[font=\footnotesize, anchor=west] at ({(\base+(0.889-0.84)*\sc+0.1)},4.175) {0.889};
  % Remove SPACING 0.903 -> 3.78cm
  \fill[gray!50] (\base,3.0) rectangle ({(\base+(0.903-0.84)*\sc)},3.35);
  \node[anchor=east, font=\footnotesize] at (\base-0.1,3.175) {Remove SPACING};
  \node[font=\footnotesize, anchor=west] at ({(\base+(0.903-0.84)*\sc+0.1)},3.175) {0.903};
  % Remove GRID 0.916 -> 4.56cm
  \fill[gray!50] (\base,2.0) rectangle ({(\base+(0.916-0.84)*\sc)},2.35);
  \node[anchor=east, font=\footnotesize] at (\base-0.1,2.175) {Remove GRID};
  \node[font=\footnotesize, anchor=west] at ({(\base+(0.916-0.84)*\sc+0.1)},2.175) {0.916};
  % Remove all ALIGNMENT 0.939 -> 5.94cm
  \fill[gray!50] (\base,1.0) rectangle ({(\base+(0.939-0.84)*\sc)},1.35);
  \node[anchor=east, font=\footnotesize] at (\base-0.1,1.175) {Remove all ALIGNMENT};
  \node[font=\footnotesize, anchor=west] at ({(\base+(0.939-0.84)*\sc+0.1)},1.175) {0.939};
  % Only ALIGNMENT 0.878 -> 2.28cm
  \fill[gray!50] (\base,0.0) rectangle ({(\base+(0.878-0.84)*\sc)},0.35);
  \node[anchor=east, font=\footnotesize] at (\base-0.1,0.175) {Only ALIGNMENT};
  ...
  \node[font=\footnotesize] at (3.3,-1.0) {Violation accuracy};
...
\begin{tikzpicture}[scale=1.0, transform shape]
  ...
  \begin{scope}[shift={(0,0)}]
    \node[font=\footnotesize] at (2.4,4.35) {Violation head};
    % joint
    \fill[blue!60] (0.6,0) rectangle (1.6,{0.876*\sc});
    \node[font=\scriptsize] at (1.1,{0.876*\sc+0.12}) {0.876};
    % violation-only
    \fill[blue!60] (2.0,0) rectangle (3.0,{0.898*\sc});
    \node[font=\scriptsize] at (2.5,{0.898*\sc+0.12}) {0.898};
    % proposal-only
    \fill[blue!60] (3.4,0) rectangle (4.4,{0.489*\sc});
    \node[font=\scriptsize] at (3.9,{0.489*\sc+0.12}) {0.489};
    % error bars (std: 0.019, 0.018, 0.071)
    ...
    \draw[thick,-{Stealth}] (0,-0.15) -- (0,3.9);
    \draw[thick,-{Stealth}] (-0.15,0) -- (4.6,0);
    \node[font=\scriptsize, align=center] at (2.5,-0.55) {joint\\violation-only\\proposal-only};
  \end{scope}
  % Right panel: proposal MSE (joint 0.051, vio-only 0.116, prop-only 0.051)
  \begin{scope}[shift={(6.6,0)}]
    \node[font=\footnotesize] at (2.4,4.35) {Proposal head};
    \fill[blue!60] (0.6,0) rectangle (1.6,{0.051*\sc*3});
    \node[font=\scriptsize] at (1.1,{0.051*\sc*3+0.12}) {0.051};
    \fill[blue!60] (2.0,0) rectangle (3.0,{0.116*\sc*3});
    \node[font=\scriptsize] at (2.5,{0.116*\sc*3+0.12}) {0.116};
    \fill[blue!60] (3.4,0) rectangle (4.4,{0.051*\sc*3});
    \node[font=\scriptsize] at (3.9,{0.051*\sc*3+0.12}) {0.051};
    % error bars (std: 0.009, 0.009, 0.010) *3
    \draw (1.1,{0.051*\sc*3+0.05}) -- (1.1,{0.051*\sc*3+0.05+0.009*\sc*3});
    \draw (2.5,{0.116*\sc*3+0.05}) -- (2.5,{0.116*\sc*3+0.05+0.009*\sc*3});
    \draw (3.9,{0.051*\sc*3+0.05}) -- (3.9,{0.051*\sc*3+0.05+0.010*\sc*3});
    \draw[thick,-{Stealth}] (0,-0.15) -- (0,3.9);
    \draw[thick,-{Stealth}] (-0.15,0) -- (4.6,0);
    \node[font=\scriptsize, align=center] at (2.5,-0.55) {joint\\violation-only\\proposal-only};
  \end{scope}
...
We compare the GNN proposal head against a nearest-neighbour baseline that
...
proximity, confirming the model learns genuine structural priors rather than
...
\begin{tikzpicture}[scale=1.0, transform shape]
  ...
  \begin{scope}
    % legend
    \fill[blue!60] (7.6,5.3) rectangle (8.0,5.55);
    \node[anchor=west, font=\scriptsize] at (8.05,5.42) {GNN (ours)};
    \fill[gray!60] (9.6,5.3) rectangle (10.0,5.55);
    \node[anchor=west, font=\scriptsize] at (10.05,5.42) {NN baseline};
    % group 1: drop 0.2
    ...
    \node[font=\footnotesize, rotate=90] at (-0.55,2.4) {Proposal IoU};
    \foreach \v in {0.05,0.10,0.15,0.20} {
      \draw (-0.1,{\v*\sc}) -- (0.1,{\v*\sc}) node[left, font=\tiny] {\v};
    }
  \end{scope}
\end{tikzpicture}
\caption{Element-completion IoU vs.\ drop ratio (mean $\pm$ std, 2 seeds).
The GNN beats the nearest-neighbour baseline when substantial structure is
...
evaluate with centre-distance matching at threshold 0.1 against ground truth
...
\begin{tikzpicture}[scale=1.0, transform shape]
  ...
  % y = value * 14 cm (0.393 -> 5.5cm)
  \def\sc{14}
  % legend
  \fill[gray!60] (5.6,4.3) rectangle (6.0,4.55);
  \node[anchor=west, font=\scriptsize] at (6.05,4.42) {VLM only};
  \fill[blue!60] (7.6,4.3) rectangle (8.0,4.55);
  \node[anchor=west, font=\scriptsize] at (8.05,4.42) {VLM + GNN};
  % Precision
  ...
  \node[font=\footnotesize, rotate=90] at (-0.55,2.4) {Score};
...
The experiments reveal a clear pattern: structural reasoning helps most when
the VLM output is \emph{sparse} (many elements missing) and less when the VLM
is already accurate. This is the expected ``Goldilocks'' behaviour --- the
graph has real information to exploit when elements are missing, and nothing
to correct when the VLM is already near-perfect. For lightweight VLMs on
realistic screenshots (which omit 10--30\% of elements), the regime where the
model helps is the regime that matters.

\subsection{Limitations}
Three limitations stand out. First, \emph{type prediction is weak}: predicting
...
Promising directions include visual feature fusion with cross-attention
(early experiments show 18--22\% proposal-MSE improvement), domain
adaptation to ScreenSpot, and extending the graph with temporal context so
the model can distinguish missing elements from elements hidden by UI state.

\section{Conclusion}
We presented a heterogeneous bipartite GraphSAGE framework that corrects
noisy GUI element predictions from lightweight VLMs. Encoding ten spatial
constraint types as a bipartite graph gives the model a strong structural
inductive bias: elements communicate only through shared constraints. The
multi-task objective couples coordinate correction, violation detection,
existence scoring, and self-supervised element completion. On RICO, the model
achieves 90--94\% violation accuracy, a $+56\%$ IoU gain over nearest-neighbour
completion at high drop ratios, and a $+2.0$pp F1 improvement on 200 real
Qwen3-VL Flash screenshots with honest checkpoint loading. The framework is
model-agnostic, adds sub-millisecond inference, and directly addresses the
dominant failure mode of lightweight VLMs --- element omission --- without
retraining them.
